...
parser.add_argument('-max_c_sequence_length', type=int, required=False, default=2000, 
					help="default: 2000")
parser.add_argument('-min_match_length', type=int, required=False, default=79, 
                    help="minimum lenght of alignment")
...
cmatch_min = args.min_match_length
...
df['sequence_uid'] = df.sample_id + "_" + df['sequence_id'].astype(str)
...
blastn_results = blastn_results.rename(columns={'sequence_id':'sequence_uid'})

df = df.merge(blastn_results, on = "sequence_uid", how = "inner")
df = df[df.length > cmatch_min]
