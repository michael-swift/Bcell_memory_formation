...
def run_gex_label_routine(adata, tissue):
    ...
    #### Scrublet ####
    # scrublet on a per sample basis
    adatas = []
    for sample in adata.obs.sample_uid.unique():
        print(f"running scrublet on {sample}")
        adata_sample = run_scrublet(adata[adata.obs.sample_uid == sample], layers=["counts", "cellbender_counts"])
        adatas = adatas.append(adata_sample)
    adata = ad.concat(adatas) 
    # Algo for definitely being a B cell
    assign_technical_categories(adata, tissue)
...
def run_celltypist(adata):
    ...
    # not sure why this isn't added automatically but need it for scanpy
...
def run_scrublet(adata, layers):
    for layer in layers:
        ...
        sns.displot(data=adata.obs, x=score, ax = axs[0])
        plt.yscale("log")
        sns.displot(data=adata.obs, x=score, kind="ecdf", ax=axs[1])
        save_figure(fig, "doublet_scores")
        ...
        doublet_clusters = selector[selector].index
        adata.obs["doublet_associated"] = adata.obs["leiden"].isin(doublet_clusters)
    ...
    save_figure(g.figure, "doublet_associated_confusion_matrix")
    return adata
...
def assign_technical_categories(adata, tissue):
    ...
    believable_leidens = permissible_leidens[permissible_leidens > 0.01].index.tolist()
    ...
    adata.obs["probable_hq_single_b_cell"] = (
        adata.obs["possible_b_cell"]
        & ~adata.obs["predicted_doublets_counts"]
        & (adata.obs["leiden"].isin(believable_leidens)) # cells in tiny clusters typically artifacts
    ...
    adata.obs["probable_hq_single_not_b_cell"] = (
        ~adata.obs["possible_b_cell"]
        & ~adata.obs["predicted_doublets_counts"]
        & (~adata.obs["leiden"].isin(believable_leidens))
        & (~adata.obs["doublet_associated"])
    )
