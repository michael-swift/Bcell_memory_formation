...
samplename=FILENAME.split("/")[-1].split('_vdj')[0]
...
fig.savefig('{}/distance_to_nearest_cdr3.pdf'.format(outdir), bbox_inches = 'tight')
plt.close(fig)
...
df.to_csv('{}/{}_vdjc_lineage_ids.tsv.gz'.format(outdir,samplename), sep = '\t', index = False)
