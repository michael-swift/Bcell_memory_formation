...
def get_r1_fqgz_using_wildcards(wildcards):
    ...
    r1 = glob(
        "{fastqs}/{sample_uid_vdj}_*R1*.fastq.gz".format(
            fastqs=config["vdj_fastq_dir"], sample_uid_vdj=wildcards["sample_uid_vdj"]
        )
    )

    r2 = glob(
        "{fastqs}/{sample_uid_vdj}_*R2*.fastq.gz".format(
            fastqs=config["vdj_fastq_dir"], sample_uid_vdj=wildcards["sample_uid_vdj"]
        )
    )
    return r1
